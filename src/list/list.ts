...
export class BaseListComponent implements OnInit, OnChanges {
  ...
  @Input() columns: BaseTableColumn<BaseModel>[] = [];
  ...
  @Input() doSearch: (model: ModelConstructor<BaseModel>, text: string, count: number) => Promise<(BaseModel & HasMacroBreakdown)[]>;

  /** Perform search filtering based on the breakdown filter meta data */
  @Input() doFilterByBreakdown: (filter: BreakdownFilterMeta[]) => Promise<(BaseModel & HasMacroBreakdown)[]>;
  ...
  @Output() select: EventEmitter<BaseModel> = new EventEmitter();
